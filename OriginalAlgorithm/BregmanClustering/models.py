...
class BregmanNodeEdgeAttributeGraphClusteringSoft( BaseEstimator, ClusterMixin ):
    ...
    def precompute_edge_divergences(self):
        if np.isnan(self.edge_means).any():
            raise ValueError("NaN in edge_means")

        K = self.n_clusters
        em = self.edge_means
        print(f"[DEBUG] precompute_edge_divergences: edge_means.shape={em.shape}, K={K}")

        # Force shape (K,K)
        if em.shape != (K, K):
            print(f"[WARN] Forcing edge_means {em.shape} -> ({K},{K})")
            em = np.full((K, K), float(em))

        edge_vals = em.reshape(-1, 1)  # (K*K,1)
        dists = pairwise_distances(
            np.array([0, 1]).reshape(-1, 1),
            edge_vals,
            metric=self.edge_divergence
        )
        self.precomputed_edge_div = dists.reshape(2, K, K)
        print(f"[DEBUG] precomputed_edge_div.shape={self.precomputed_edge_div.shape}")
    ...
    def vector_to_membership(self, z_vec, K):
        z_int = np.asarray(z_vec, dtype=int)
        N = z_int.shape[0]
        Z = np.zeros((N, K), dtype=float)
        Z[np.arange(N), z_int] = 1.0
        return Z
    ...
    def fit(self, edge_index, X, Y, Z_init=None):
        ...
        if Z_init is None:
            ...
            if Z_init.ndim == 1:
                self.predicted_memberships = self.vector_to_membership(Z_init, self.n_clusters)
            else:
                self.predicted_memberships = np.asarray(Z_init, dtype=float)

        # Final sanity: Z must be (N,K)
        Z = self.predicted_memberships
        if Z.shape != (self.N, self.n_clusters):
            raise ValueError(
                f"predicted_memberships has shape {Z.shape}, expected ({self.N}, {self.n_clusters})"
            )

        # mmap buffer and proceed
        path = tempfile.mkdtemp()
        Zpath = os.path.join(path, 'z.mmap')
        self.Ztilde = np.memmap(Zpath, dtype=float, shape=(self.N, self.n_clusters), mode='w+')

        self.M_projection(X_, Y, Z)
        convergence = False
        iteration = 0
        old_log_prob = np.inf
        while not convergence:
            ...
            convergence = self.stop_criterion(X_, Y, self.predicted_memberships, Z_new,
                                            old_log_prob, new_log_prob, iteration)
    ...
    def initialize(self, edge_index, E, Y):
        self.edge_index = edge_index if self.edge_index is None else self.edge_index

        if self.use_random_init:
            # N x K one-hot random memberships
            z0 = np.random.randint(self.n_clusters, size=Y.shape[0])
            self.predicted_memberships = self.vector_to_membership(z0, self.n_clusters)
            self.memberships_from_graph = z0
            self.memberships_from_attributes = z0
            self.graph_init = False
            return

        # Otherwise, use your initializer but validate shape
        ...
        model.initialize(edge_index, E, Y)
        Zp = np.asarray(model.predicted_memberships, dtype=float)

        if Zp.ndim == 1:  # label vector
            Zp = self.vector_to_membership(Zp, self.n_clusters)

        if Zp.shape[0] != self.N or Zp.shape[1] != self.n_clusters:
            raise ValueError(
                f"Initializer returned memberships with shape {Zp.shape}, "
                f"expected ({self.N}, {self.n_clusters})."
            )

        self.predicted_memberships = Zp
        self.memberships_from_graph = frommembershipMatriceToVector(model.memberships_from_graph)
        self.memberships_from_attributes = frommembershipMatriceToVector(model.memberships_from_attributes)
        self.graph_init = False
    ...
    def computeAttributeMeans(self, Y, Z):
        ...
        # Z: (N,K), Y: (N,m)
        nk = Z.sum(axis=0).astype(float) + 10 * np.finfo(float).eps  # (K,)
        attribute_means = (Z.T @ Y) / nk[:, None]                    # (K,m)
        if np.isnan(attribute_means).any():
            raise ValueError("attribute_means contains NaN")
        return attribute_means
